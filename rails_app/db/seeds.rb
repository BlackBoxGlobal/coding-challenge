...
actors.each do |a|
  actor = Actor.create(a.except(:characters))
  next unless a[:characters]

  a[:characters].each do |character_attr|
    actor.characters << Character.where(character_attr).first_or_create
  end
  actor.save
end
