...
episodes = [
  ...
  {
    title: 'Lisa’s Date With Destiny',
    main: 'Lisa'
  },
  {
    title: 'Barts comet',
...
puts 'Write a simple ruby script to reformat the data to:
{
  bart: {
...
hash = episodes.each_with_object({}) do |episode, new_hash|
  new_hash[episode[:main].downcase.to_sym] ||= { number_of_episodes: 0 }
  new_hash[episode[:main].downcase.to_sym][:number_of_episodes] += 1
end
